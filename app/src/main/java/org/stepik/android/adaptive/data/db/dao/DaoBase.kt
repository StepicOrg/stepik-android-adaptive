...
abstract class DaoBase<T>(private val databaseOperations: DatabaseOperations): IDao<T> {
    ...
    override fun get(whereColumnName: String, whereValue: String): Single<RxOptional<T>> =
            getAll("SELECT * FROM ${getDbName()} WHERE $whereColumnName = ? COUNT 1", arrayOf(whereValue)).map { RxOptional(it.firstOrNull()) }
